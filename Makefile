...
VERSION ?= $(shell git describe --tags --always 2>/dev/null | cut -d"v" -f2)
LDFLAGS ?= -ldflags=-X=sigs.k8s.io/karpenter/pkg/operator.Version=$(VERSION)

# Docker Hub repo for `make build` (ko --bare uses this as the full image name).
KO_DOCKER_REPO ?= docker.io/gflorian/karpenter-provider-digital-ocean
...
.PHONY: help local_build build test vet tidy generate install-crds run

help: ## Show this help
	@awk 'BEGIN {FS = ":.*##"; printf "Usage:\n  make \033[36m<target>\033[0m\n"} /^[a-zA-Z_-]+:.*?##/ { printf "  \033[36m%-12s\033[0m %s\n", $$1, $$2 }' $(MAKEFILE_LIST)

local_build: ## Build controller binary to bin/
	CGO_ENABLED=0 GOARCH=$(GOARCH) go build $(LDFLAGS) -o bin/karpenter-provider-digital-ocean ./cmd/controller

build: ## Build and push the controller image to Docker Hub with ko
	@command -v ko >/dev/null || { echo "ko is required: go install github.com/ko-build/ko@latest"; exit 1; }
	CGO_ENABLED=0 GOFLAGS="$(GOFLAGS) $(LDFLAGS)" KO_DOCKER_REPO="$(KO_DOCKER_REPO)" \
		ko build --bare --tags "$(IMAGE_TAGS)" ./cmd/controller
...
run: local_build install-crds ## Run controller using DIGITALOCEAN_TOKEN from DOKS_TOKEN_FILE
